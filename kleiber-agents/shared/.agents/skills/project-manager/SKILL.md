...
- Remember that MCP calls may serialize from a single agent flow. For true parallel work, orchestrate subagents inside the harness first, then let each subagent use its own MCP-backed session if needed.
...
7. For parallel execution, do not assume one agent issuing multiple MCP calls will stay parallel. Prefer harness-level subagents for the fan-out, then let each subagent use its own Kleiber MCP session where needed.
8. When practical, isolate parallel workers using separate git branches and worktrees to reduce collisions.
9. Keep an execution log in the conversation and summarize completed, blocked, and deferred tasks.
