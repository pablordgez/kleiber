...
```bash
/path/to/coding-agent-pack-v2/install.sh --mode project
```

Or explicitly:

```bash
/path/to/coding-agent-pack-v2/install.sh --mode project --root /path/to/project
```

By default the installer uses symlinks where practical so the shared skill remains canonical. To copy instead:

```bash
/path/to/coding-agent-pack-v2/install.sh --mode project --copy
```

## Global install

```bash
/path/to/coding-agent-pack-v2/install.sh --mode global
```
